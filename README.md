...
4. Настройка окружения и системы
Для работы приложения необходимо настроить файлы в папке json_documents под названиями "config.json" и "requests.json". 
4.1 Config.json
Первый json отвечает за базовую настройку приложения и выглядит примерно так:
```json5
{
    "config": {
        "max_responses": 10,
...
Первая часть (config) отвечает за название проекта, версию приложения и максимальное число ответов по поиску. 
Вторая часть содержит список файлов (необязательно существующих), в которых будет осуществляться поиск по содержимому. По умолчанию они расположены в папке Database, к которой ведёт путь из конструктора.
Впишите сюда файлы, в которых следует осуществлять поиск.
Если в папке не окажется ни одного файла, то система создаст тестовые экземпляры для проверки.

4.2 Requests_json
Второй документ содержит список запросов, которые поисковая система будет стараться отыскать. Впишите сюда слова или строки, которые механизм должен найти в предложенных файлах базы данных.
```json5
{
    "requests": [
        "первый тестовый",
...
Ответы находятся в папке Json_documents.
